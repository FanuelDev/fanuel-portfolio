  ...
  <section id="business" ref="sectionEl">
    <div class="container biz-wrap">
      <div class="biz-content" ref="contentEl">
        <div class="section-tag">{{ t('business.tag') }}</div>
        <h2 class="section-title left-title">
          {{ t('business.title') }}<br />
          <span class="gold">{{ t('business.title2') }}</span>
        </h2>
        <p>{{ t('business.p1') }}</p>
        <p>{{ t('business.p2') }}</p>
        <div class="services" ref="servicesEl">
          <div class="service" v-for="(s, i) in tm('business.services')" :key="i">
      ...
      <div class="biz-visual" ref="visualEl">
        ...
        <div class="biz-float-card">
          <div class="float-icon"><AppIcon name="target" :size="22" /></div>
          <div>
            <div class="float-title">{{ t('business.float_title') }}</div>
            <div class="float-sub">{{ t('business.float_sub') }}</div>
          </div>
        </div>
      </div>
    </div>
...
const servicesEl = ref(null); const visualEl = ref(null)
...
onMounted(() => {
  ...
  gsap.to('.biz-float-card', { y: -8, duration: 2.5, ease: 'sine.inOut', yoyo: true, repeat: -1 })
})
</script>

<style scoped>
#business { padding: 7rem 0; }
.biz-wrap { display: grid; grid-template-columns: 1fr 1fr; gap: 4rem; align-items: center; }
...
.service {
  ...
  background: var(--card); border: 1px solid rgba(212,168,83,0.18);
  border-radius: 14px; padding: 1.1rem 1.25rem;
  transition: border-color 0.3s, background 0.3s, transform 0.3s;
}
.service:hover { border-color: var(--gold); background: var(--gold-lo); transform: translateX(4px); }
.service-num { font-family: 'Space Mono', monospace; font-size: 0.7rem; color: var(--gold); padding-top: 0.15rem; min-width: 28px; }
...
.biz-glow { position: absolute; width: 110%; height: 110%; top: -5%; left: -5%; background: radial-gradient(circle, rgba(212,168,83,0.08) 0%, transparent 65%); pointer-events: none; }
.biz-photo { position: relative; border-radius: 22px; overflow: hidden; border: 1px solid rgba(212,168,83,0.22); box-shadow: 0 0 60px rgba(212,168,83,0.12); }
.biz-photo img { width: 100%; display: block; }
.biz-photo-overlay { position: absolute; inset: 0; background: linear-gradient(to top, var(--bg) 0%, transparent 45%); }
.biz-float-card {
...
.float-icon { display: flex; color: var(--purple); }
